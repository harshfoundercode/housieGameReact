...
export default function TambolaLive({ gameId }) {
  ...
  // Keep flyRef in sync with fly state so handleFlyDone always sees latest
  useEffect(() => {
    flyRef.current = fly;
  }, [fly]);

  /* ── Trigger animations when a new number comes in ── */
  // pendingNum holds the number that arrived from socket but hasn't started
  // its animation chain yet
  ...
  const triggerAnimationForNumber = useCallback((n) => {
    ...
    // After the big ball has been shown, start the fly sequence
    setTimeout(() => {
      setShowPulse(false);
      setShowParticles(false);
      setBigNum(null);
  ...
  /* ── Socket fly-done releases busyRef ── */
  // We override handleFlyDone to also release busyRef when toSlot0 finishes
  function handleFlyDoneSocket() {
    const f = flyRef.current;
    setFly(null);
    if (!f) return;
    ...
    if (f.type === "toBoard") {
      ...
      setTray(p => { const t = [...p]; t[0] = f.num; return t; });
      setTrayKey(p => { const k = [...p]; k[0]++; return k; });
      // Release busy after ball lands in slot0
      setTimeout(() => {
        busyRef.current = false;
        // If another number arrived while we were busy, process it now
        if (pendingNumRef.current !== null) {
          const next = pendingNumRef.current;
          pendingNumRef.current = null;
          triggerAnimationForNumber(next);
        }
      }, 1200);
    }
  }
  ...
  useEffect(() => {
    ...
    socket.on("game_over", () => { setGameStatus("over"); setDone(true); });

    socket.on("number_called", (data) => {
      if (data.game_id !== gameId) return;
      const number = data.number;

      // Update calledCount immediately for stats display
      ...
      // If animation is running, queue this number
      if (busyRef.current) {
        pendingNumRef.current = number;
        return;
      }
    ...
    socket.on("old_numbers", (data) => {
      if (data.calledNumbers && data.calledNumbers.length > 0) {
        ...
        // Populate tray with last two numbers (no animation)
        if (nums.length >= 2) {
  ...
  }, [gameId, triggerAnimationForNumber]);
      ...
      <div ref={containerRef} style={{
        ...
        {fly && (
          <FlyingBall
            ...
            onDone={handleFlyDoneSocket} duration={fly.duration}
          />
        )}
