...
// ───────────────────────────────────────────── */
// const DECADE_COLORS = [
//   { base: "#004296", light: "#1a6fd8", mid: "#002b66", glow: "rgba(0,66,150,0.8)" },
//   { base: "#005f8a", light: "#0090cc", mid: "#003a55", glow: "rgba(0,95,138,0.8)" },
//   { base: "#1a5276", light: "#2e86c1", mid: "#0f2e45", glow: "rgba(26,82,118,0.8)" },
//   { base: "#1a7a6a", light: "#1abc9c", mid: "#0d4d42", glow: "rgba(26,122,106,0.8)" },
//   { base: "#b8860b", light: "#FBEFA4", mid: "#7a5a05", glow: "rgba(251,239,164,0.8)" },
//   { base: "#c9a227", light: "#ffe066", mid: "#8a6d10", glow: "rgba(201,162,39,0.8)" },
//   { base: "#a07620", light: "#d4a017", mid: "#6b4f10", glow: "rgba(160,118,32,0.8)" },
//   { base: "#7d5a0b", light: "#b8860b", mid: "#4d3608", glow: "rgba(125,90,11,0.7)" },
//   { base: "#3a3a8c", light: "#6666cc", mid: "#1e1e5a", glow: "rgba(58,58,140,0.8)" },
// ];
...
// function BigBall({ number, size = 148, animationPhase }) {
//   const hasNum = number !== null && number !== undefined;
//   const fontSize = size * 0.36;
//   const c = hasNum ? dc(number) : null;
...
//           transform: "scale(0)",
//           opacity: 0,
//           animation: "tl-bingoEnter 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards",
//         };
//       case "visible":
//         return {
//           transform: "scale(1)",
...
//           transform: "scale(1)",
//           opacity: 1,
//           animation: "tl-bingoExit 0.5s ease-in forwards",
//         };
//       default:
//         return {
//           transform: "scale(0)",
...
//           filter: "blur(25px)",
//           opacity: animationPhase === "visible" ? 0.7 : animationPhase === "entering" ? 0 : 0,
...
//           : "radial-gradient(circle at 35% 25%, #1e2a4a, #001433)",
//         boxShadow: hasNum
//           ? `0 0 30px ${c.glow}, 0 0 60px ${c.glow.replace("0.8", "0.4")}, 0 0 100px ${c.glow.replace("0.8", "0.2")}, inset -8px -8px 22px rgba(0,0,0,0.5), inset 6px 6px 16px rgba(255,255,255,0.2)`
//           : "inset -8px -8px 22px rgba(0,0,0,0.6)",
//         display: "flex",
//         alignItems: "center",
//         justifyContent: "center",
...
//             <div style={{
//               position: "absolute",
//               top: size * 0.08, left: size * 0.16,
//               width: size * 0.35, height: size * 0.20,
//               background: "rgba(255,255,255,0.28)",
//               borderRadius: "50%",
//               transform: "rotate(-30deg)",
//               filter: `blur(${size < 60 ? 2 : 5}px)`,
//               pointerEvents: "none",
//               zIndex: 2,
//             }} />
//             <div style={{
//               position: "absolute",
//               top: size * 0.14, left: size * 0.20,
//               width: size * 0.18, height: size * 0.09,
...
//           color: "#fff",
//           textShadow: hasNum
//             ? `0 0 10px ${c.glow}, 0 2px 12px rgba(0,0,0,0.8)`
...
//   ══════════════════════════════════════════ */
//   const ENTER_DURATION  = 800;   // Ball zoom in time
//   const VISIBLE_DURATION = 2800;  // Ball stays visible
//   const EXIT_DURATION   = 500;   // Ball zoom out time
//   const GAP_DURATION    = 400;   // Gap between exit and next enter
//   // Total cycle: 800 + 2800 + 500 + 400 = 4500ms per number
...
//     return () => events.forEach(event => document.removeEventListener(event, unlockAudio));
//   }, []);
...
//   // Run one complete animation cycle for a number
//   const runAnimationCycle = useCallback((number) => {
//     clearAllTimers();

//     // Update display: prev gets current latest, latest gets new number
//     setPrevNum(latestNumRef.current);
//     setLatestNum(number);
    
//     // Phase 1: ENTER (ball zooms in)
//     setBallPhase("entering");
//     playSoundOnly(number);

//     // Phase 2: VISIBLE (ball stays)
//     const t1 = setTimeout(() => {
//       setBallPhase("visible");
//     }, ENTER_DURATION);
//     phaseTimersRef.current.push(t1);

//     // Phase 3: EXIT (ball zooms out)
//     const t2 = setTimeout(() => {
//       setBallPhase("exiting");
//     }, ENTER_DURATION + VISIBLE_DURATION);
//     phaseTimersRef.current.push(t2);

//     // Phase 4: HIDDEN then process next
//     const t3 = setTimeout(() => {
//       setBallPhase("hidden");
      
//       // Process next after gap
//       const t4 = setTimeout(() => {
//         if (queueRef.current.length > 0) {
//           const nextNumber = queueRef.current.shift();
//           runAnimationCycle(nextNumber);
//         } else {
//           isProcessingRef.current = false;
//         }
//       }, GAP_DURATION);
...
//       if (isProcessingRef.current) {
//         // Currently animating, add to queue
//         queueRef.current.push(lastCalledNum);
//       } else {
//         // Start immediately
//         isProcessingRef.current = true;
//         runAnimationCycle(lastCalledNum);
//       }
//     }
//   }, [lastCalledNum, runAnimationCycle]);
...
//       display: "grid", gridTemplateColumns: "repeat(10,1fr)",
//       gap: mobile ? 3 : isNarrow ? 5 : 7,
//       padding: mobile ? 6 : isNarrow ? 8 : 12, width: "100%",
//     }}>
//       {Array.from({ length: 90 }, (_, i) => {
//         const n = i + 1;
//         return (
//           <div key={n} style={{ display: "flex", alignItems: "center", justifyContent: "center", aspectRatio: "1" }}>
//             <GridBall number={n} called={calledSet.has(n)} size={GRID} />
//           </div>
//         );
//       })}
//     </div>
...
//       <div style={{
//         width: TRAY_SLOT, height: TRAY_SLOT, borderRadius: "50%",
//         background: "rgba(0,10,30,0.55)", border: "1px solid rgba(251,239,164,0.10)",
//         boxShadow: "inset 0 2px 10px rgba(0,0,0,0.5)",
//         display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
//       }}>
//         <TrayBall number={slotNum} size={TRAY} />
//       </div>
//       <span style={{ fontSize: 7, color: "rgba(251,239,164,0.25)", letterSpacing: 1, fontFamily: "'Cinzel',serif" }}>
//         {label}
//       </span>
//     </div>
//   );
...
//   const BigBallArea = () => (
//     <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
//       <BigBall number={latestNum} size={BIG} animationPhase={ballPhase} />
//     </div>
//   );
...
//       ].map(s => (
//         <div key={s.lbl} style={{
//           flex: 1, textAlign: "center", padding: compact ? "5px 4px" : "9px 6px",
//           borderRadius: 10, background: "rgba(0,20,51,0.45)", border: "1px solid rgba(251,239,164,0.09)",
//         }}>
//           <div style={{ fontSize: compact ? 16 : 22, fontWeight: 900, fontFamily: "'Cinzel',serif", color: s.color, lineHeight: 1 }}>
//             {s.val}
//           </div>
...
//       display: "flex", alignItems: "center", gap: 8,
//       padding: "6px 14px", borderRadius: 20,
//       background: "rgba(0,20,51,0.60)", border: "1px solid rgba(251,239,164,0.12)",
//     }}>
//       <div style={{
//         width: 8, height: 8, borderRadius: "50%",
//         background: connected ? (gameStatus === "started" ? "#1abc9c" : "#FBEFA4") : "#ff4444",
...
//         /* BALL ENTER - Zoom in from 0 to 1 with bounce */
//         @keyframes tl-bingoEnter {
...
//         /* BALL EXIT - Zoom out from 1 to 0 */
//         @keyframes tl-bingoExit {
...
//               display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
//               gap: 20, padding: "20px 14px",
//               borderRight: "1px solid rgba(251,239,164,0.06)", background: "rgba(0,0,0,0.18)",
//             }}>
//               <div style={{ textAlign: "center" }}>
//                 <div style={{ fontSize: 7, color: "rgba(251,239,164,0.38)", letterSpacing: 5, marginBottom: 5 }}>✦ LIVE DRAW ✦</div>
//                 <h1 className="tl-shimmer-text" style={{ fontSize: 13, fontWeight: 900, letterSpacing: 4, fontFamily: "'Cinzel',serif" }}>TAMBOLA</h1>
...
//               display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
//               gap: 28, padding: "30px 22px",
//               borderRight: "1px solid rgba(251,239,164,0.07)", background: "rgba(0,0,0,0.16)",
//               backdropFilter: "blur(6px)",
//             }}>
//               <div style={{ textAlign: "center" }}>
//                 <div style={{ fontSize: 7, color: "rgba(251,239,164,0.40)", letterSpacing: 5, marginBottom: 6 }}>✦ LIVE DRAW ✦</div>
...
//           }}>
//             <div style={{ fontSize: isMobile ? 24 : 36, fontWeight: 900, fontFamily: "'Cinzel',serif", color: "#FBEFA4", letterSpacing: 4 }}>
//               FULL HOUSE
//             </div>
//             <div style={{ fontSize: 9, color: "rgba(255,255,255,0.28)", letterSpacing: 4 }}>ALL 90 NUMBERS CALLED</div>
//             <button
    ...
    <div style={{
      ...
      {hasNum && animationPhase !== "hidden" && (
        <div style={{
          ...
          background: `radial-gradient(circle, ${c.glow} 0%, transparent 70%)`,
          filter: "blur(25px)",
          opacity: animationPhase === "visible" ? 0.7 : 0,
          transition: "opacity 0.5s ease",
      ...
      <div style={{
        ...
        {hasNum && (
          <>
            <div style={{ position: "absolute", top: size*0.08, left: size*0.16, width: size*0.35, height: size*0.20, background: "rgba(255,255,255,0.28)", borderRadius: "50%", transform: "rotate(-30deg)", filter: `blur(${size<60?2:5}px)`, pointerEvents: "none", zIndex: 2 }} />
            <div style={{ position: "absolute", top: size*0.14, left: size*0.20, width: size*0.18, height: size*0.09, background: "rgba(255,255,255,0.50)", borderRadius: "50%", transform: "rotate(-30deg)", filter: "blur(1px)", pointerEvents: "none", zIndex: 2 }} />
          </>
        )}
        <span style={{
          fontSize, fontWeight: 900, fontFamily: "'Cinzel', serif", color: "#fff",
          textShadow: hasNum ? `0 0 10px ${c.glow}, 0 2px 12px rgba(0,0,0,0.8)` : "0 2px 12px rgba(0,0,0,0.8)",
          zIndex: 3, lineHeight: 1, position: "relative",
        }}>
          {hasNum ? number : ""}
        </span>
      </div>
...
/* ── TrayBall with Animation ── */
function TrayBall({ number, size = 64, isNew = false, isMoving = false }) {
  const ballRef = useRef(null);

  // Trigger animation when marked as new or moving
  useEffect(() => {
    if (ballRef.current && (isNew || isMoving)) {
      const el = ballRef.current;
      el.style.animation = 'none';
      el.offsetHeight;
      
      if (isNew) {
        el.style.animation = 'tl-trayPop 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards';
      } else if (isMoving) {
        el.style.animation = 'tl-traySlide 0.5s ease-out forwards';
      }
    }
  }, [number, isNew, isMoving]);
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: "50%",
        background: ballGradient(number),
        boxShadow: `0 0 15px ${c.glow.replace("0.8", "0.4")}, inset -3px -3px 10px rgba(0,0,0,0.45), inset 2px 2px 7px rgba(255,255,255,0.18), 0 4px 12px rgba(0,0,0,0.4)`,
        display: "flex", alignItems: "center", justifyContent: "center",
        position: "relative", flexShrink: 0,
        willChange: "transform, opacity",
      }}
    ...
    <div style={{
      ...
      {called && (
        <div style={{
          ...
          transform: "rotate(-30deg)", filter: "blur(1.5px)", pointerEvents: "none",
        }} />
      )}
      <span style={{
        fontSize: size*0.33, fontWeight: 800,
...
export default function TambolaLive({
  ...
  useEffect(() => { setCalledSet(new Set(calledNumbers)); }, [calledNumbers]);
  useEffect(() => {
    if (gameStatus === "over") setDone(true);
    if (gameStatus === "started") setDone(false);
  }, [gameStatus]);
  ...
  // Track which ball is new and which is moving
  ...
  const playSoundOnly = useCallback((number) => {
    try {
      if (audioRef.current) { audioRef.current.pause(); audioRef.current = null; }
      const audio = new Audio(`/sounds/${number}.mp3`);
      audio.volume = 1.0; audio.preload = 'auto';
      audioRef.current = audio;
      audio.play().catch(() => {});
      audio.onended = () => { audioRef.current = null; };
      audio.onerror = () => { audioRef.current = null; };
  ...
  const runAnimationCycle = useCallback((number) => {
    ...
    // PREV gets current latest (with slide animation)
    ...
    if (currentLatest !== null) {
      ...
      setTimeout(() => setPrevIsMoving(false), 600);
    }

    // LATEST gets new number (with pop animation)
    setLatestIsNew(true);
    setLatestNum(number);
    // Reset new flag after animation
    setTimeout(() => setLatestIsNew(false), 700);
    ...
    // Phase: VISIBLE
    const t1 = setTimeout(() => setBallPhase("visible"), ENTER_DURATION);
    phaseTimersRef.current.push(t1);

    // Phase: EXIT
    const t2 = setTimeout(() => setBallPhase("exiting"), ENTER_DURATION + VISIBLE_DURATION);
    phaseTimersRef.current.push(t2);
  ...
  /* ══ Sub-components ══ */
  const BoardGrid = ({ mobile = false }) => (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(10,1fr)", gap: mobile?3:isNarrow?5:7, padding: mobile?6:isNarrow?8:12, width: "100%" }}>
      {Array.from({ length: 90 }, (_, i) => {
        const n = i + 1;
        return (
          <div key={n} style={{ display: "flex", alignItems: "center", justifyContent: "center", aspectRatio: "1" }}>
  ...
  const TraySlot = ({ slotNum, label, vertical, isNew, isMoving }) => (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: vertical ? 4 : 6 }}>
      <div style={{
        width: TRAY_SLOT, height: TRAY_SLOT, borderRadius: "50%",
        background: "rgba(0,10,30,0.55)", border: "1px solid rgba(251,239,164,0.10)",
        ...
        <TrayBall number={slotNum} size={TRAY} isNew={isNew} isMoving={isMoving} />
      </div>
      <span style={{ fontSize: 7, color: "rgba(251,239,164,0.25)", letterSpacing: 1, fontFamily: "'Cinzel',serif" }}>
        {label}
      </span>
  ...
  const TraySlots = ({ vertical = false }) => (
    <div style={{ display: "flex", flexDirection: vertical ? "column" : "row", gap: 18, alignItems: "center" }}>
      <TraySlot slotNum={latestNum} label="LATEST" vertical={vertical} isNew={latestIsNew} />
      <TraySlot slotNum={prevNum} label="PREV" vertical={vertical} isMoving={prevIsMoving} />
    </div>
  );
  ...
  const StatsBar = ({ compact = false }) => (
    <div style={{ display: "flex", gap: 8, width: "100%" }}>
      {[
        ...
        <div key={s.lbl} style={{ flex:1, textAlign:"center", padding:compact?"5px 4px":"9px 6px", borderRadius:10, background:"rgba(0,20,51,0.45)", border:"1px solid rgba(251,239,164,0.09)" }}>
          <div style={{ fontSize:compact?16:22, fontWeight:900, fontFamily:"'Cinzel',serif", color:s.color, lineHeight:1 }}>{s.val}</div>
          <div style={{ fontSize:7, color:"rgba(255,255,255,0.28)", letterSpacing:2, marginTop:3 }}>{s.lbl}</div>
        </div>
      ))}
    </div>
  ...
  const ProgressBar = () => (
    <div style={{ width: "100%" }}>
      <div style={{ width:"100%", height:3, borderRadius:2, background:"rgba(251,239,164,0.08)", overflow:"hidden" }}>
        <div style={{ height:"100%", borderRadius:2, background:"linear-gradient(90deg,#004296,#b8860b,#FBEFA4)", width:`${pct}%`, transition:"width 0.8s cubic-bezier(0.4,0,0.2,1)", boxShadow:"0 0 8px rgba(251,239,164,0.55)" }} />
      </div>
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
        <span style={{ fontSize:7, color:"rgba(255,255,255,0.18)", fontFamily:"'Cinzel',serif" }}>0</span>
        <span style={{ fontSize:7, color:"rgba(251,239,164,0.6)", fontFamily:"'Cinzel',serif" }}>{pct}%</span>
  ...
  const StatusDot = () => (
    <div style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 14px", borderRadius:20, background:"rgba(0,20,51,0.60)", border:"1px solid rgba(251,239,164,0.12)" }}>
      <div style={{ width:8, height:8, borderRadius:"50%", background:connected?(gameStatus==="started"?"#1abc9c":"#FBEFA4"):"#ff4444", boxShadow:connected?`0 0 8px ${gameStatus==="started"?"#1abc9c":"#FBEFA4"}`:"0 0 8px #ff4444" }} />
      <span style={{ fontSize:9, color:"rgba(255,255,255,0.70)", fontFamily:"'Cinzel',serif", letterSpacing:1.5 }}>
        {connected ? gameStatus.toUpperCase() : "OFFLINE"}
      </span>
    </div>
  ...
  return (
    <>
      <style>{`
        ...
        /* TRAY BALL - Pop in for NEW number */
        @keyframes tl-trayPop {
          0%   { transform: scale(0); opacity: 0; }
        ...
        /* TRAY BALL - Slide down for MOVING number */
        @keyframes tl-traySlide {
      ...
      <div ref={containerRef} style={{
        width: "100%", overflow: "hidden", borderRadius: 18,
        background: `radial-gradient(ellipse at 15% 0%, rgba(0,66,150,0.35) 0%, transparent 55%), radial-gradient(ellipse at 85% 100%,rgba(251,239,164,0.07) 0%, transparent 50%), linear-gradient(160deg,#002b66 0%,#001433 100%)`,
        border: "1px solid rgba(251,239,164,0.10)",
        boxShadow: "0 0 0 1px rgba(0,66,150,0.40), 0 30px 80px rgba(0,8,25,0.85)",
        color: "#fff", position: "relative", fontFamily: "'Raleway',sans-serif",
      }}>
        <div style={{ position: "absolute", top: 14, right: 14, zIndex: 20 }}><StatusDot /></div>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 0, backgroundImage: `linear-gradient(rgba(251,239,164,0.025) 1px,transparent 1px), linear-gradient(90deg,rgba(251,239,164,0.025) 1px,transparent 1px)`, backgroundSize: "52px 52px" }} />
        ...
        {isMobile && (
          <div style={{ display:"flex", flexDirection:"column", padding:"14px 10px 20px", gap:12, position:"relative", zIndex:1 }}>
            ...
            <ProgressBar />
            <div style={{ background:"rgba(0,0,0,0.22)", borderRadius:12, border:"1px solid rgba(251,239,164,0.05)" }}><BoardGrid mobile /></div>
          </div>
        )}
        ...
        {!isMobile && isNarrow && (
          <div style={{ display:"flex", alignItems:"stretch", position:"relative", zIndex:1, minHeight:500 }}>
            <div style={{ flexShrink:0, width:LEFT_W, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20, padding:"20px 14px", borderRight:"1px solid rgba(251,239,164,0.06)", background:"rgba(0,0,0,0.18)" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:7, color:"rgba(251,239,164,0.38)", letterSpacing:5, marginBottom:5 }}>✦ LIVE DRAW ✦</div>
                <h1 className="tl-shimmer-text" style={{ fontSize:13, fontWeight:900, letterSpacing:4, fontFamily:"'Cinzel',serif" }}>TAMBOLA</h1>
              </div>
              <BigBallArea /><TraySlots /><StatsBar /><ProgressBar />
            </div>
            <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", padding:"16px 14px 14px 12px" }}>
              <div style={{ flex:1, background:"rgba(0,8,24,0.35)", borderRadius:14, border:"1px solid rgba(251,239,164,0.05)", display:"flex", alignItems:"center", overflow:"hidden" }}><BoardGrid /></div>
            </div>
          </div>
        )}
        ...
        {!isMobile && !isNarrow && (
          <div style={{ display:"flex", alignItems:"stretch", position:"relative", zIndex:1, minHeight:560 }}>
            <div style={{ flexShrink:0, width:LEFT_W, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28, padding:"30px 22px", borderRight:"1px solid rgba(251,239,164,0.07)", background:"rgba(0,0,0,0.16)", backdropFilter:"blur(6px)" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:7, color:"rgba(251,239,164,0.40)", letterSpacing:5, marginBottom:6 }}>✦ LIVE DRAW ✦</div>
                <h1 className="tl-shimmer-text" style={{ fontSize:18, fontWeight:900, letterSpacing:4, fontFamily:"'Cinzel',serif" }}>TAMBOLA</h1>
                <div style={{ fontSize:7, color:"rgba(255,255,255,0.18)", letterSpacing:3, marginTop:4, fontFamily:"'Cinzel',serif" }}>90 BALL · LIVE DRAW</div>
        ...
        {/* DONE */}
        {done && (
          <div style={{ position:"absolute", inset:0, zIndex:50, background:"rgba(0,8,25,0.92)", backdropFilter:"blur(14px)", borderRadius:18, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:16 }}>
            <div style={{ fontSize:isMobile?24:36, fontWeight:900, fontFamily:"'Cinzel',serif", color:"#FBEFA4", letterSpacing:4 }}>FULL HOUSE</div>
            <div style={{ fontSize:9, color:"rgba(255,255,255,0.28)", letterSpacing:4 }}>ALL 90 NUMBERS CALLED</div>
            <button onClick={() => window.location.reload()} style={{ marginTop:12, padding:"12px 36px", fontSize:10, fontWeight:700, fontFamily:"'Cinzel',serif", letterSpacing:3, background:"linear-gradient(135deg,#FBEFA4,#c9b86c)", color:"#001433", border:"none", borderRadius:30, cursor:"pointer", boxShadow:"0 4px 24px rgba(251,239,164,0.38)" }}>PLAY AGAIN</button>
          </div>
        )}
      </div>
    </>
