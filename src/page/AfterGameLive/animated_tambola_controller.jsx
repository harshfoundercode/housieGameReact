...
/* ─────────────────────────────────────────────
   MAIN COMPONENT
───────────────────────────────────────────── */
export default function TambolaLive({ gameId = 34 }) {
  // Socket state
  ...
  useEffect(() => {
    ...
    // Number called event - THIS IS THE MAIN EVENT
    socket.on("number_called", (data) => {
      ...
      // Play sound for the number
      playNumberSound(number);

      // Show the number in big display with animations
      setBigNum(number);
      setBigKey(k => k + 1);
      setShowPulse(true);
      setShowParticles(false);

      // Trigger particles after a small delay
      setTimeout(() => setShowParticles(true), 200);

      // Update called set and count
      setCalledSet(prev => {
      ...
      // Mark the cell as arriving (for grid animation)
      ...
      setTray(prev => {
        const newTray = [...prev];
        // Shift: slot0 -> slot1, new number -> slot0
        newTray[1] = prev[0];
        newTray[0] = number;
        return newTray;
      });
    ...
    // Old numbers event (for reconnection/refresh)
    socket.on("old_numbers", (data) => {
      console.log("📜 Old numbers received:", data.calledNumbers?.length, "numbers");

      if (data.calledNumbers && data.calledNumbers.length > 0) {
        const newSet = new Set(data.calledNumbers);
        setCalledSet(newSet);
        setCalledCount(data.calledNumbers.length);

        // Set last two numbers in tray
    ...
    <div style={{ display: "flex", flexDirection: vertical ? "column" : "row", gap: 14, alignItems: "center" }}>
      {[0, 1].map(i => (
        ...
        >
          ...
          {tray[i] !== null
            ? <div key={`${i}-${trayKey[i]}`}><TrayBall number={tray[i]} size={TRAY_BALL_SIZE} isNew={i === 0} /></div>
            : <div style={{
              width: TRAY_BALL_SIZE * 0.4, height: TRAY_BALL_SIZE * 0.4,
              borderRadius: "50%",
              background: "rgba(251,239,164,0.03)",
