...
export const useCheckoutModal = (gameId, apiPricing) => {
  const [showCheckout, setShowCheckout] = useState(false);
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState(null);
  const [walletBalance, setWalletBalance] = useState(0);
  const [walletLoading, setWalletLoading] = useState(false);
  ...
  // ✅ Helper function to prepare ticket_ids with per ticket pricing
  const prepareTicketIdsPayload = useCallback((cartItems) => {
    if (!cartItems || cartItems.length === 0) return [];

    console.log("📊 prepareTicketIdsPayload - cartItems:", cartItems);
    console.log("📊 prepareTicketIdsPayload - apiPricing:", apiPricing);

    // ✅ Extract pricing from apiPricing (DYNAMIC - API se aa raha hai)
    const ticketPrice = parseFloat(apiPricing?.ticket_price) || 15;
    const halfSheetPrice = parseFloat(apiPricing?.half_sheet_price) || 5;
    const fullSheetPrice = parseFloat(apiPricing?.full_sheet_price) || 10;

    console.log("🎯 Using DYNAMIC Pricing:", { ticketPrice, halfSheetPrice, fullSheetPrice });
      ...
      while (i < rowTickets.length) {
        ...
        if (i + 5 < rowTickets.length && 
          ...
          if (isConsecutive) {
            const sheetTickets = rowTickets.slice(i, i + 6).map(t => t.ticket);
            const ticketType = 'fullsheet';
            // ✅ Full Sheet: total price divided by 6
            const perTicketPrice = parseFloat((fullSheetPrice / 6).toFixed(2));
            
            console.log(`📊 Fullsheet - Total: ${fullSheetPrice}, Per Ticket: ${perTicketPrice}`);
        ...
        if (i + 2 < rowTickets.length && 
          ...
          if (isConsecutive) {
            const sheetTickets = rowTickets.slice(i, i + 3).map(t => t.ticket);
            const ticketType = 'halfsheet';
            // ✅ Half Sheet: total price divided by 3
            const perTicketPrice = parseFloat((halfSheetPrice / 3).toFixed(2));
            
            console.log(`📊 Halfsheet - Total: ${halfSheetPrice}, Per Ticket: ${perTicketPrice}`);
        ...
        const ticketType = 'random';
        const ticketId = parseInt(ticket.id || ticket.ticket_id || ticket.ticketId || 0);
        // ✅ Random ticket: use ticketPrice
        const perTicketPrice = ticketPrice;
  ...
  const handleDirectPayment = useCallback(async (cartTotal, cartItems, onSuccess, ticketDetails) => {
    ...
    try {
      ...
      // ✅ Prepare ticket details with correct pricing
