...
const GamePage = () => {
  ...
  // ✅ DYNAMIC PRICING: Pass both gameId and apiPricing
  const checkoutModal = useCheckoutModal(gameId, apiPricing);
  ...
  // ✅ Handle checkout payment - DYNAMIC PRICING
  const handleCheckoutPayment = () => {
    if (!checkoutModal.selectedPaymentMethod) {
      alert("Please select a payment method");
      return;
    ...
    // ✅ Prepare ticket details with DYNAMIC pricing from apiPricing
    const ticketDetails = checkoutModal.prepareTicketIdsPayload(cartModal.cart);
    console.log("📊 Prepared Ticket Details with DYNAMIC Pricing:", JSON.stringify(ticketDetails, null, 2));
  ...
  // ✅ Handle agent contact - DYNAMIC PRICING
  const handleAgentContact = (agent) => {
    if (cartModal.cart.length === 0) {
      alert("Your cart is empty!");
      return;
  ...
  return (
    <>
      ...
      <CheckoutModal
        ...
        prepareTicketIdsPayload={checkoutModal.prepareTicketIdsPayload}
        onPaymentComplete={handleCheckoutPayment}
      />
