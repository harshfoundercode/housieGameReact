...
//                             <div className="w-20 h-12 bg-linear-to-br from-[#004296] to-[#0066cc] rounded-xl flex items-center justify-center shadow-lg">
...
const Footer = () => {
    ...
    // Extract value from malformed JSON using regex
    ...
    // Parse contact data from string (handles malformed JSON)
    const parseContactData = (contactString) => {
        if (!contactString) return null;
        
        // If it's already an object, return it
        if (typeof contactString === 'object') return contactString;
        
        // Try direct JSON parse first
        try {
            return JSON.parse(contactString);
        } catch (e) {
            // Use regex to extract values from malformed JSON
            console.log("Using regex fallback for contact data");
            return {
                address: extractValue(contactString, 'address') || "Delhi, India",
                email: extractValue(contactString, 'email') || "test@mail.com",
                phone: extractValue(contactString, 'phone') || "9999999999",
                whatsapp: extractValue(contactString, 'whatsapp') || "7881116009"
            };
        }
    };
    ...
    useEffect(() => {
        const fetchContactInfo = async () => {
            try {
                ...
                if (result.success && result.data && result.data.contact) {
                    const contactData = parseContactData(result.data.contact);
                    ...
                    <div className="text-center sm:text-left">
                        ...
                        {isLoadingContact ? (
                            ...
                            <ul className="space-y-3">
                                ...
                                <li className="flex items-center gap-3 justify-center sm:justify-start">
                                    <span className="text-[#FBEFA4] text-lg">📧</span>
                                    <a href={`mailto:${contactInfo.email}`} className="text-white/60 hover:text-[#FBEFA4] text-sm transition-colors">
                                        {contactInfo.email}
                                    </a>
                                </li>
                                <li className="flex items-center gap-3 justify-center sm:justify-start">
                                    <span className="text-[#FBEFA4] text-lg">📞</span>
                                    <a href={`tel:${contactInfo.phone}`} className="text-white/60 hover:text-[#FBEFA4] text-sm transition-colors">
                                        {formatPhone(contactInfo.phone)}
                                    </a>
                                </li>
                                <li className="flex items-center gap-3 justify-center sm:justify-start">
                                    ...
                                    >
                                        <span>WhatsApp: {contactInfo.whatsapp}</span>
                                        <span className="text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full font-medium">Chat</span>
                                    </a>
                                </li>
                            </ul>
                        )}
