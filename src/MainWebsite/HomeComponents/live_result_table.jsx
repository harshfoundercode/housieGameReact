...
//                                     });
//                                 }}
//                                 className={`cursor-pointer p-3 sm:p-4 md:p-0 hover:bg-gray-50 transition-all ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'
//                                     }`}
//                             >
//                                 {/* Mobile View */}
...
const LiveResultTable = ({ limit = 4, showViewAll = true }) => {
    ...
    // Helper function to check if game is completed (past)
    const isGameCompleted = (startDateTime) => {
        if (!startDateTime) return false;
        ...
        try {
            ...
            // Game is completed if more than 30 minutes past
    ...
    // Navigate to game based on status
    const handleGameClick = (game) => {
        if (game.status === 'live' || game.status === 'completed') {
            // Navigate to live result page
    ...
    // Navigate to live draw for current live game
    const handleViewLiveDraw = () => {
        if (liveGames.length > 0) {
            // If there's a live game, go directly to it
            const liveGame = liveGames[0];
            navigate(ROUTES.AFTERGAME, {
                state: {
                    gameId: liveGame.id,
                    gameName: liveGame.gameName,
                    gameDate: liveGame.gameDate,
                    roundTime: liveGame.roundTime
                }
            });
        } else {
            // If no live games, show all games page
            navigate(ROUTES.ALL_LIVE_RESULTS);
        }
    };
    ...
    return (
        <section className="pt-6 sm:pt-8 md:pt-10 lg:pt-12 px-3 sm:px-4">
            <div className="max-w-sm sm:max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-auto">
                ...
                {/* View Live Draw Button - Show when there are live games */}
                {/* {liveGames.length > 0 && (
                    <div className="text-center pt-2 sm:pt-3 mb-6">
                        <button
                            onClick={handleViewLiveDraw}
                            className="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 sm:px-8 py-2.5 sm:py-3 rounded-full font-semibold text-sm sm:text-base shadow-md sm:shadow-lg hover:shadow-lg sm:hover:shadow-xl hover:scale-105 transition-all duration-300 inline-flex items-center gap-1.5 sm:gap-2 animate-pulse"
                        >
                ...
                {/* View All Results Button - Show on All Games page */}
                {/* {!showViewAll && (
                    <div className="text-center pt-2 sm:pt-3 mb-6">
                        <button
                            onClick={() => navigate(ROUTES.AFTERGAME)}
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                                                ...
                                                {item.status === 'live' ? 'LIVE' : item.status === 'completed' ? 'ENDED' : 'UPCOMING'}
                                            </span>
                                            <span className="bg-[#FBEFA4] text-[#004296] px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold shadow-sm">
                                                {item.drawTime}
                                            </span>
