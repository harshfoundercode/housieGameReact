...
const Register = () => {
    ...
    // Get phone from location state (if coming from login page)
    const [formData, setFormData] = useState({
        phone: location.state?.phone || "",
        referralCode: referralCodeFromUrl || "", // ✅ Auto-fill from URL
        firstName: "",
        lastName: "",
    });
    ...
    // ✅ Show notification if referral code is present
    React.useEffect(() => {
        if (referralCodeFromUrl) {
            setSuccessMessage(`🎉 Referral code ${referralCodeFromUrl} applied!`);
    ...
    const handleVerifyOTP = async () => {
        ...
        try {
            ...
            setSuccessMessage(`Welcome ${formData.firstName}! Registration successful!`);
            
            // ✅ Navigate to home page after successful registration
            setTimeout(() => {
                navigate("/");
                // Optional: Refresh to update navbar state
    ...
    return (
        <div className="min-h-screen bg-linear-to-br from-[#004296] via-[#002b66] to-[#001433] flex flex-col lg:flex-row">
            ...
            {/* LEFT PANEL - Static Image & Content */}
            ...
            <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-6 sm:py-8 md:py-10 min-h-screen">
                <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg">
                    ...
                    <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-5 sm:p-6 md:p-8 border-2 border-[#FBEFA4]/30 transition-all duration-300">
                        ...
                        {/* Success Message */}
                        {successMessage && (
                            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl text-green-600 text-xs sm:text-sm flex items-center gap-2">
                        ...
                        {step === 1 && (
                            <>
                             {/* ✅ Referral Code Banner (if from URL) */}
                                {referralCodeFromUrl && (
                                    <div className="mb-4 p-3 bg-[#FBEFA4] border border-[#FFE44D] rounded-xl text-[#004296] text-xs sm:text-sm flex items-center gap-2">
                                        <span>🎯</span> 
                                        <span>
                                            <strong>Referral Code Applied:</strong> {referralCodeFromUrl}
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className={`w-full pl-11 pr-4 py-3 sm:py-3.5 rounded-xl border-2 outline-none transition-all text-sm sm:text-base text-gray-800 uppercase ${
                                                referralCodeFromUrl 
                                                    ? 'border-green-500 bg-green-50 text-green-700' 
                                                    : 'border-gray-200 focus:border-[#004296] focus:ring-2 focus:ring-[#004296]/20'
                                            }`}
                                            readOnly={!!referralCodeFromUrl} // ✅ Read-only if from URL
                                        />
                                        {referralCodeFromUrl && (
                                            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-green-500 text-xs font-medium">
                                                ✓ Applied
                                            </span>
                                        )}
                                    </div>
                                    {referralCodeFromUrl && (
                                        <p className="text-green-600 text-[10px] sm:text-xs mt-1 ml-1">
