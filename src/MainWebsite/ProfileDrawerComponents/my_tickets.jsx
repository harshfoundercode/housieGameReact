...
//                         let parsedTicketIds = [];
//                         try {
//                             // Parse ticket_ids if it's a string
//                             if (typeof req.ticket_ids === 'string') {
//                                 parsedTicketIds = JSON.parse(req.ticket_ids);
//                             } else if (Array.isArray(req.ticket_ids)) {
//                                 parsedTicketIds = req.ticket_ids;
//                             }
//                         } catch (e) {
//                             console.error("Error parsing ticket_ids for request", req.request_id, e);
//                             parsedTicketIds = [];
//                         }
...
//                             ticket_ids_array: parsedTicketIds
//                         };
//                     });
...
//     // Calculate total amount from ticket_ids
//     const calculateTotalAmount = (ticketIds) => {
//         if (!ticketIds || ticketIds.length === 0) return 0;
//         return ticketIds.reduce((total, ticket) => {
//             return total + (ticket.ticket_amount || 0);
//         }, 0);
//     };

//     // Get ticket type display
//     const getTicketTypeDisplay = (ticketIds) => {
//         if (!ticketIds || ticketIds.length === 0) return 'N/A';
//         const types = [...new Set(ticketIds.map(t => t.ticket_name))];
//         return types.join(', ');
//     };
...
//                                                                     {formatDate(booking.game_date) || "N/A"} { booking.game_time || "N/A"} 
//                                                                 </span>
//                                                             </div>
//                                                             <div className="flex items-center gap-2">
//                                                                 <span className="text-gray-400">🎫</span>
...
//                                         const totalAmount = calculateTotalAmount(request.ticket_ids_array);
//                                         const ticketTypes = getTicketTypeDisplay(request.ticket_ids_array);
...
//                                                                 <span className="text-gray-600">
//                                                                     Tickets: {request.ticket_ids_array?.length || 0}
//                                                                 </span>
//                                                             </div>
//                                                             <div className="flex items-center gap-2">
//                                                                 <span className="text-gray-400">📅</span>
...
//                                     <p className="font-medium text-[#004296] font-bold">
//                                         ₹{calculateTotalAmount(selectedRequest.ticket_ids_array)}
//                                     </p>
//                                 </div>
//                                 <div>
//                                     <p className="text-gray-500 text-xs">Ticket Types</p>
//                                     <p className="font-medium text-gray-800">
//                                         {getTicketTypeDisplay(selectedRequest.ticket_ids_array)}
//                                     </p>
//                                 </div>
...
//                             {/* ✅ Ticket IDs Display - Fixed to render properly */}
//                             {selectedRequest.ticket_ids_array && selectedRequest.ticket_ids_array.length > 0 && (
//                                 <div className="mb-6">
//                                     <h4 className="font-bold text-gray-800 mb-3">
//                                         Ticket Details ({selectedRequest.ticket_ids_array.length})
//                                     </h4>
...
//                                                 {selectedRequest.ticket_ids_array.map((ticket, idx) => (
//                                                     <tr key={ticket.ticket_id || idx} className="hover:bg-gray-50">
//                                                         <td className="px-4 py-3 text-sm text-gray-500">{idx + 1}</td>
//                                                         <td className="px-4 py-3 text-sm font-medium text-gray-900">#{ticket.ticket_id}</td>
//                                                         <td className="px-4 py-3 text-sm">
//                                                             <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
//                                                                 ticket.ticket_name === 'fullsheet' ? 'bg-green-100 text-green-700' :
//                                                                 ticket.ticket_name === 'halfsheet' ? 'bg-blue-100 text-blue-700' :
//                                                                 'bg-yellow-100 text-yellow-700'
//                                                             }`}>
//                                                                 {ticket.ticket_name}
//                                                             </span>
//                                                         </td>
//                                                         <td className="px-4 py-3 text-sm font-medium text-gray-900">₹{ticket.ticket_amount}</td>
//                                                     </tr>
//                                                 ))}
//                                             </tbody>
...
//                                                     <td className="px-4 py-3 text-sm font-bold text-[#004296]">
//                                                         ₹{calculateTotalAmount(selectedRequest.ticket_ids_array)}
//                                                     </td>
//                                                 </tr>
//                                             </tfoot>
...
//                             {selectedRequest.status === 'accepted' && selectedRequest.ticket_ids_array?.length > 0 && (
...
const MyTickets = () => {
    ...
    // Group tickets by booking_id using API total_price
    const groupTicketsByBooking = (ticketData) => {
        const grouped = {};
        ...
        ticketData.forEach(ticket => {
            ...
            if (!grouped[bookingId]) {
                grouped[bookingId] = {
                    ...
                    game_time: ticket.game_time || null,
                    tickets: [],
                    total_price: 0  // Will be calculated from API total_price
                };
            }
            ...
            // ✅ Use API total_price for each ticket
            const ticketTotalPrice = parseFloat(ticket.total_price) || 0;
            ...
            grouped[bookingId].tickets.push({
                ...
                ticket_data: parsedTicketData,
                ticket_status: ticket.ticket_status || 'sold',
                total_price: ticketTotalPrice  // ✅ Store per ticket total_price from API
            });
            
            // ✅ Add to booking total price
            grouped[bookingId].total_price += ticketTotalPrice;
        });
    ...
    // Calculate total amount from ticket_ids array
    const calculateTotalAmount = (ticketIds) => {
        if (!ticketIds || ticketIds.length === 0) return 0;
        return ticketIds.reduce((total, ticket) => {
            return total + (parseFloat(ticket.ticket_amount) || 0);
        }, 0);
    };
                                                ...
                                                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-gray-400">💰</span>
                                                                <span className="text-[#004296] font-bold">₹{booking.total_price.toFixed(2)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {booking.tickets.map((ticket, idx) => (
                                                            <div key={idx} className="flex items-center gap-1 bg-gray-50 px-2 py-1 rounded-lg">
                                                                ...
                                                                <span className="text-xs text-gray-400 ml-1">₹{ticket.total_price.toFixed(2)}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                        ...
                        <div className="p-5 md:p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 rounded-xl">
                                ...
                                <div>
                                    <p className="text-gray-500 text-xs">Total Amount</p>
                                    <p className="font-medium text-[#004296] font-bold">
                                        ₹{calculateTotalAmount(selectedRequest.ticket_ids_array).toFixed(2)}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-gray-500 text-xs">Ticket Types</p>
                                            ...
                                            <tbody className="bg-white divide-y divide-gray-200">
                                                {selectedRequest.ticket_ids_array.map((ticket, idx) => (
                                                    <tr key={ticket.ticket_id || idx} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-3 text-sm font-medium text-gray-900">₹{parseFloat(ticket.ticket_amount).toFixed(2)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                            <tfoot className="bg-gray-50">
                        ...
                        <div className="p-5 md:p-6">
                            ...
                            <div className="mb-5 flex items-center gap-3 flex-wrap">
                                ...
                                <span className="text-xs bg-green-100 text-green-600 px-3 py-1 rounded-full font-medium">
                                    ₹{selectedTicketDetail.total_price.toFixed(2)}
                                </span>
                            </div>
                            ...
                            <div className="space-y-4">
                                {selectedTicketDetail.tickets.map((ticket, idx) => (
                                    <div key={idx} className="border border-gray-200 rounded-xl p-4">
                                        ...
                                        {/* ✅ Show ticket amount from API */}
                                        <div className="text-sm text-gray-600 mb-2">
                                            Amount: <span className="font-bold text-[#004296]">₹{ticket.total_price.toFixed(2)}</span>
                                        </div>
