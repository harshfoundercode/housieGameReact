...
//                       </label>
//                       <input type="text" name="upi_id" value={formData.upi_id} onChange={handleChange} placeholder="Enter UPI ID number" className={`w-full px-4 py-3 rounded-lg border ${errors.upi_id ? 'border-red-500 bg-red-50' : 'border-gray-300'} focus:border-[#004296] outline-none transition-all`} />
...
const KYCAddEdit = ({ isEditMode = false, existingData = null }) => {
  ...
  const validateUPI = (upiId) => {
    ...
    // Check if @ is present only once
    const atCount = (upiId.match(/@/g) || []).length;
    if (atCount !== 1) {
      return "UPI ID must contain exactly one @ symbol";
    }
    
    // Check if username part is not empty
    const [username, domain] = upiId.split('@');
    if (!username || username.length < 2) {
      return "Username part of UPI ID is too short";
    }
    
    // Check if domain part is valid
    if (!domain || domain.length < 3) {
      return "Domain part of UPI ID is invalid";
    }
    
    // Common valid UPI domains
    const validDomains = ['okhdfcbank', 'okicici', 'oksbi', 'okaxis', 'ybl', 'ibl', 'paytm', 'upi'];
    if (!validDomains.includes(domain.toLowerCase()) && !domain.includes('.')) {
      // Allow custom domains with dots (e.g., username@icici, username@hdfc)
      if (domain.length < 3) {
  ...
  const handleChange = (e) => {
    ...
    // UPI ID ke liye real-time validation
    if (name === "upi_id") {
      const trimmedValue = value.trim();
      setFormData(prev => ({ ...prev, upi_id: trimmedValue }));
      
      // Real-time UPI validation
      if (trimmedValue.length > 0) {
        const upiError = validateUPI(trimmedValue);
        if (upiError) {
