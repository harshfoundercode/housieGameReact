...
const RulesAndTerms = () => {
    ...
    const safeJSONParse = (jsonString) => {
        ...
        try {
            ...
            console.warn("Direct JSON parse failed, attempting to clean string...");
            try {
                ...
                let cleanedString = jsonString
                    ...
                    .replace(/,\s*]/g, ']')           // Remove trailing commas before ]
                    .replace(/"\s*\n\s*"/g, '","')    // Fix unquoted newlines between values
    ...
    // Parse HTML content to extract list items from specific section
    const parseSectionItems = (htmlContent, sectionTitle) => {
        if (!htmlContent) return [];
        ...
        // Find the specific h3 section
        const h3Elements = doc.querySelectorAll('h3');
        let foundSection = false;
        
        for (let h3 of h3Elements) {
            if (h3.textContent.includes(sectionTitle)) {
                foundSection = true;
                let nextElement = h3.nextElementSibling;
                
                // Collect all list items and paragraphs until next h3
                while (nextElement && nextElement.tagName !== 'H3') {
                    if (nextElement.tagName === 'UL') {
                        const listItems = nextElement.querySelectorAll('li');
                        listItems.forEach(li => {
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    if (result.data.contact) {
                        if (typeof result.data.contact === 'string') {
                            contactData = safeJSONParse(result.data.contact);
                        } else if (typeof result.data.contact === 'object') {
    ...
    const dosAndDontsList = parseSectionItems(apiData.rules, "Do's & Don'ts");
    const rulesGuidelinesList = parseSectionItems(apiData.rules, "Rules & Guidelines");
    const termsConditionsList = parseSectionItems(apiData.terms, "Terms & Conditions");
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    {rulesGuidelinesList.length > 0 && (
                        <div className="mb-10">
                            ...
                            <div className="space-y-3">
                                {rulesGuidelinesList
                                    .filter(item => !item.includes('DISCLAIMER'))
                                    .slice(0, showMoreRules ? rulesGuidelinesList.length : 8)
                                    .map((item, index) => (
                                    <div key={index} className="flex gap-3">
                                        <span className="text-[#004296] font-bold min-w-7">
                                            {index + 1}.
                    ...
                    {/* Disclaimer */}
                    {rulesGuidelinesList.some(item => item.includes('DISCLAIMER')) && (
                        <div className="mb-10 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                            <p className="text-yellow-800 text-sm">
                                {rulesGuidelinesList.find(item => item.includes('DISCLAIMER'))}
                            </p>
                        </div>
                    )}
                    ...
                    {termsConditionsList.length > 0 && (
                        <div className="mb-10">
                            ...
                            <div className="space-y-3">
                                {termsConditionsList
                                    .slice(0, showMoreTerms ? termsConditionsList.length : 6)
                                    .map((item, index) => (
                                    <div key={index} className="flex gap-3">
                                        <span className="text-[#004296] font-bold min-w-7">
                                            {index + 1}.
